...
# Supported Kivy version required for operation. Older version may work
# too, but they're not supported. You can remove or modify this setting
# at your own risk.
kivy.require('1.10.1')
...
Config.set('kivy', 'window_icon', './Images/power-on-white.png')
...
USER_SETTINGS_FILE = os.path.join(FILE_PATH, FILE_NAME)
...
class ImminentPopup(Popup):
    ...
    The user is warned they are about to force the selected command
    to be executed and asked to confirm this action by selecting a
    button: Yes or No.
    """
    ...
    def __init__(self, cmd):
        """Initialize ImminentPopup class."""
...
class FinalPopup(Popup):
    """
    A popup presented at the end of the countdown just prior command execution.
    ...
    def __init__(self):
        """Initialize the FinalPopup class."""
        super(FinalPopup, self).__init__()
        self.anim = None

    def start_final_timer(self):
        """Start the countdown to cmd execution and App auto-closing."""
        ...
        App.get_running_app().root.show_app_window()
...
class LinShutdownTimer(GridLayout, ToggleButtonBehavior):
    ...
    def __init__(self):
        """Initialize the LinShutdownTimer class."""
        ...
        self.cmd = None
        self.time = None
        self.imminent_popup = None
        self.final_popup = None

    def apply_defaults(self):
        """Apply default values from USER_SETTINGS_FILE."""
    ...
    def _get_time(self):
        """Get the current button selection from the preset time buttons."""
    ...
    def _get_curr_settings(self):
        ...
        """
        self.user_settings['default_cmd'] = self._get_cmd().lower()
        self.user_settings['default_time'] = self._get_time()
    ...
    def _on_keyboard_down(self, keyboard, keycode, text, modifiers):
        ...
        """
        if self.preset_keybinding_enabled is True:
            # cmd buttons
            if keycode[0] == 115:
    ...
    def _toggle_keybinding_allowed(self):
    ...
    def _initiate_shutdown(self, *args):
    ...
    def _set_timer(self, button_time):
        """
    ...
    def _toggle_sub_time_status(self):
    ...
    def _toggle_add_time_status(self):
        """Enable/Disable the '+15 min' button."""
        ...
        # make it active
        elif self.countdown == 0 or self.add_time_disabled is True:
            self.add_time_disabled = False

    def _toggle_start_pause_status(self):
    ...
    def _toggle_preset_status(self):
        """
    ...
    def _toggle_start_pause_state(self):
        """
    ...
    def _reset_preset_state(self):
        """Return the preset time buttons to the normal (up) state."""
    ...
    def _toggle_start_pause_text(self):
        """
        Toggle the 'Start/Pause'.

        Switches the button's text label between "Start" and "Pause".
        """
    ...
    def _start_stop_timer(self):
        ...
        if self.ids.start_pause.state == 'down' and self.countdown > 0:
            ...
            # shutdown process
            self.anim.bind(on_complete=self._initiate_shutdown)
            # Start the animation
            self.anim.start(self)

    def _add_time(self, button_time):
        """
        + 15 minutes to the countdown timer.
    ...
    def _sub_time(self, button_time):
        """
        - 15 minutes off the countdown timer.
    ...
    def _reset(self):
        ...
        Animation.cancel_all(self),
        self._clear_timer(),
        self._toggle_start_pause_status(),
        self._toggle_start_pause_text(),
        self._toggle_start_pause_state(),
        self._toggle_preset_status(),
        self._toggle_cmd_status(),
        self._toggle_sub_time_status(),
        self._toggle_abort_status(),
        self._toggle_keybinding_allowed(),
        self.apply_defaults()
...
class LinShutdownApp(App):
    ...
    def build(self):
        """Instatiates the App class and returns an instance to run."""
